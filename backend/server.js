...
const ExcelJS = require("exceljs"); // Replaces vulnerable xlsx (CVE-2023, CVE-2024)
...
// Mongoose: enforce strict queries to prevent unintended schema mismatches (#12)
...
// Hide framework fingerprint (#13)
app.disable("x-powered-by");

// Trust the first proxy (Render, Heroku, etc.) so secure cookies work (#8)
app.set("trust proxy", 1);

// Generic error response — never leak stack traces or driver messages (#15)
...
// ObjectId validator middleware (#5) — apply to any route with :id
...
// Security headers (#9). CSP relaxed for inline handlers used by the existing
// frontend; tighten over time. allow Google Fonts + same-origin assets.
app.use(
  helmet({
    contentSecurityPolicy: {
      ...
      directives: {
        "default-src": ["'self'"],
        "script-src": ["'self'", "'unsafe-inline'"], // legacy inline onclicks
        // Override Helmet default of 'none' so existing inline handlers
        // (onclick=, onsubmit=, onchange=) still fire. Future cleanup:
        // migrate to addEventListener and tighten this back to 'none'.
        "script-src-attr": ["'unsafe-inline'"],
        "style-src": ["'self'", "'unsafe-inline'", "https://fonts.googleapis.com"],
        "font-src": ["'self'", "https://fonts.gstatic.com", "data:"],
        "img-src": ["'self'", "data:", "https:"],
        "connect-src": ["'self'"],
        "frame-src": ["'self'"], // allow PDF preview iframe (same-origin)
    ...
    crossOriginEmbedderPolicy: false, // PDF iframe compatibility
...
// CORS — explicit origin, never wildcard with credentials (#14)
...
const roomSchema = new mongoose.Schema({
  ...
  createdBy: { type: String, index: true }, // (#11) email of faculty who owns it
});
...
const allocationSchema = new mongoose.Schema({
  ...
  attendance: [
    {
      roomNo: String,
      // (#zip-feature) status flow: draft → saved → finalized.
      // When status === 'finalized', the API blocks any further write.
...
app.use(
  session({
    ...
    cookie: {
      ...
      httpOnly: true,             // (#8) JS cannot read the cookie
      secure: IS_PROD,            // (#8) only over HTTPS in production
      sameSite: "lax",            // (#8/#17) blocks most cross-site CSRF
    },
  })
);
...
// ============================================================
// CSRF PROTECTION (#17)
// Double-submit token: server issues a per-session token via /api/csrf-token,
// frontend echoes it back in the X-CSRF-Token header on state-changing calls.
// SameSite=lax cookie covers the common browser cross-site cases; this header
// check covers the rest (e.g. tools that bypass SameSite).
// ============================================================
...
// Frontend reads this once on load, then sends it back on every write.
...
// (#11) Identifies the faculty who owns a record. Falls back to "unknown" only
// for legacy records — new writes always set this.
function userKey(req) {
  return (req.user && req.user.emails && req.user.emails[0] && req.user.emails[0].value) || null;
}
...
// Logout (#16) — must be POST so a malicious site can't trigger it via <img>.
// Protected by CSRF token. We also keep a GET handler that returns 405 with a
// hint so old links don't silently 404.
app.post("/auth/logout", requireCsrf, (req, res) => {
  req.logout(() => {
    req.session.destroy(() => {
      res.clearCookie("aiml.sid");
...
// (#7) Apply hard size + count limits and a strict file-type filter so a
// malicious upload can't OOM the server or smuggle non-spreadsheet payloads.
// ============================================================

const ALLOWED_EXCEL_EXT = /\.(xlsx|xls|csv)$/i;
...
const upload = multer({
  ...
  limits: {
    ...
    files: 20,                 // up to 20 course files per request
    fields: 200,
  },
  fileFilter: (req, file, cb) => {
    const okExt = ALLOWED_EXCEL_EXT.test(file.originalname || "");
    const okMime = ALLOWED_EXCEL_MIME.has(file.mimetype);
    if (okExt && okMime) return cb(null, true);
    cb(new Error("Only .xlsx, .xls, or .csv files are allowed"));
  },
});

// Express handler for multer errors — return clean JSON instead of crashing.
function handleUploadErrors(err, req, res, next) {
  if (err && (err instanceof multer.MulterError || /allowed/i.test(err.message))) {
...
// GET all rooms — shared across all faculty
app.get("/api/rooms", isLoggedIn, async (req, res) => {
  try {
    const rooms = await Room.find().sort({ roomNo: 1 });
    res.json(rooms);
  } catch (err) {
    sendServerError(res, err, "GET /api/rooms");
  }
});

// POST add a new room
app.post("/api/rooms", isLoggedIn, requireCsrf, async (req, res) => {
  try {
    const { roomNo, benches, enabled } = req.body;
    if (typeof roomNo !== "string" || !roomNo.trim()) {
      return res.status(400).json({ error: "roomNo is required" });
    }
    const benchCount = Number(benches);
    if (!Number.isFinite(benchCount) || benchCount < 1 || benchCount > 1000) {
      return res.status(400).json({ error: "benches must be a positive number" });
    }
    const room = new Room({
      roomNo: roomNo.trim(),
      benches: benchCount,
      enabled: !!enabled,
      createdBy: userKey(req),
    });
    await room.save();
    ...
    sendServerError(res, err, "POST /api/rooms");
  }
});

// PUT update a room — shared, id-validated (#5)
app.put("/api/rooms/:id", isLoggedIn, requireCsrf, validateObjectId(), async (req, res) => {
  try {
    const { roomNo, benches, enabled } = req.body;
    ...
    sendServerError(res, err, "PUT /api/rooms/:id");
  }
});

// DELETE a room — shared, id-validated (#5)
app.delete("/api/rooms/:id", isLoggedIn, requireCsrf, validateObjectId(), async (req, res) => {
  try {
    const result = await Room.findByIdAndDelete(req.params.id);
    if (!result) return res.status(404).json({ error: "Room not found" });
    res.json({ message: "Room deleted" });
  } catch (err) {
    sendServerError(res, err, "DELETE /api/rooms/:id");
  }
});
  ...
  async (req, res) => {
    try {
      ...
      for (let idx = 0; idx < courses.length; idx++) {
        ...
        try {
          // (#4) Parse with exceljs (replaces vulnerable xlsx). Supports xlsx
          // and xls (via xlsx engine); csv handled inline below.
          const isCsv = /\.csv$/i.test(file.originalname || "");
          let students = [];

          if (isCsv) {
            // Lightweight CSV parsing — safe for the simple Name,USN format.
            ...
            if (nameIdx === -1 || usnIdx === -1) {
              throw new Error("CSV must have 'Name' and 'USN' columns");
            }
            for (let r = 1; r < lines.length; r++) {
              const cols = lines[r].split(",");
              const name = (cols[nameIdx] || "").trim();
              const usn = (cols[usnIdx] || "").trim();
            ...
            if (!headerRow) throw new Error("Empty worksheet");

            const headers = {};
            headerRow.eachCell({ includeEmpty: false }, (cell, colNumber) => {
              const v = String(cell.value || "").trim().toLowerCase();
              if (v === "name") headers.name = colNumber;
              if (v === "usn") headers.usn = colNumber;
            });
            if (!headers.name || !headers.usn) {
              throw new Error("Sheet must have 'Name' and 'USN' columns");
            }

            sheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
              if (rowNumber === headerRow.number) return; // skip header
              const nameCell = row.getCell(headers.name);
              const usnCell = row.getCell(headers.usn);
              const name = String(nameCell.value || "").trim();
              const usn = String(usnCell.value || "").trim();
              if (name && usn) students.push({ name, usn });
            });
          }
      ...
      // --- Fetch enabled rooms from DB, sorted (shared across faculty) ---
      ...
      // --- If editing, delete the old allocation first (zip-merged) ---
      // The Edit / Re-generate button on the output preview sets replaceId so
      // the new allocation supersedes the old one in history.
      const replaceId = req.body.replaceId;
      if (replaceId && mongoose.isValidObjectId(replaceId)) {
        try {
      ...
      sendServerError(res, err, "POST /api/allocate");
    }
  }
);
...
// GET all past allocations (list view) — shared across faculty
app.get("/api/history", isLoggedIn, async (req, res) => {
  try {
    ...
    sendServerError(res, err, "GET /api/history");
  }
});

// GET a specific allocation by ID (full detail) — shared, id-validated (#5)
app.get("/api/history/:id", isLoggedIn, validateObjectId(), async (req, res) => {
  try {
    const allocation = await Allocation.findById(req.params.id);
    if (!allocation) return res.status(404).json({ error: "Not found" });
    res.json(allocation);
  } catch (err) {
    sendServerError(res, err, "GET /api/history/:id");
  }
});

// DELETE an allocation — shared, id-validated, CSRF-protected.
// (zip-merged feature) Any logged-in faculty can remove a past allocation.
app.delete("/api/history/:id", isLoggedIn, requireCsrf, validateObjectId(), async (req, res) => {
  try {
    const result = await Allocation.findByIdAndDelete(req.params.id);
    if (!result) return res.status(404).json({ error: "Not found" });
    res.json({ message: "Allocation deleted" });
  } catch (err) {
    sendServerError(res, err, "DELETE /api/history/:id");
  }
});
...
app.post("/api/attendance/save", isLoggedIn, requireCsrf, async (req, res) => {
  try {
    const { allocationId, roomNo, students, status } = req.body;

    if (!allocationId || !roomNo || !Array.isArray(students)) {
      return res.status(400).json({ error: "allocationId, roomNo, and students are required." });
    }
    if (!mongoose.isValidObjectId(allocationId)) {
      return res.status(400).json({ error: "Invalid allocationId" });
    }

    // Default to "draft" for any unknown / missing status string.
    const validStatus = ["draft", "saved", "finalized"].includes(status) ? status : "draft";

    // Shared across faculty — any logged-in user can save attendance
    ...
    // Block any change to a finalized room — finalize is one-way.
    ...
    sendServerError(res, err, "POST /api/attendance/save");
  }
});

// --- PDF: Absent Report ---
app.get("/api/pdf/absent-report/:id", isLoggedIn, validateObjectId(), async (req, res) => {
  try {
    ...
    sendServerError(res, err, "PDF absent-report");
  }
});

// --- PDF: Absent Report per Course ---
app.get("/api/pdf/absent-report/:id/course/:courseIdx", isLoggedIn, validateObjectId(), async (req, res) => {
  try {
    const alloc = await Allocation.findById(req.params.id);
    ...
    sendServerError(res, err, "PDF absent-report/course");
  }
});

// --- PDF: Absent Report per Room ---
app.get("/api/pdf/absent-report/:id/room/:roomNo", isLoggedIn, validateObjectId(), async (req, res) => {
  try {
    const alloc = await Allocation.findById(req.params.id);
    if (!alloc) return res.status(404).json({ error: "Allocation not found" });
    if (!alloc.attendance || alloc.attendance.length === 0)
    ...
    sendServerError(res, err, "PDF absent-report/room");
  }
});
...
app.get("/api/pdf/notice/:id", isLoggedIn, validateObjectId(), async (req, res) => {
  try {
    const alloc = await Allocation.findById(req.params.id);
    if (!alloc) {
      return res.status(404).json({ error: "Allocation not found" });
    ...
    sendServerError(res, err, "PDF notice");
  }
});
...
app.get("/api/pdf/seating/:id", isLoggedIn, validateObjectId(), async (req, res) => {
  try {
    const alloc = await Allocation.findById(req.params.id);
    if (!alloc) {
      return res.status(404).json({ error: "Allocation not found" });
    ...
    sendServerError(res, err, "PDF seating");
  }
});

// --- PDF: Attendance Sheet ---
app.get("/api/pdf/attendance/:id", isLoggedIn, validateObjectId(), async (req, res) => {
  try {
    const alloc = await Allocation.findById(req.params.id);
    if (!alloc) {
      return res.status(404).json({ error: "Allocation not found" });
    ...
    sendServerError(res, err, "PDF attendance");
  }
});
...
app.post("/api/attendance/absent-pdf", isLoggedIn, requireCsrf, async (req, res) => {
  try {
    ...
    sendServerError(res, err, "PDF absent-report-detailed");
  }
});

// ============================================================
// 404 for unknown /api/* routes (#18)
// Without this, the SPA catch-all below would return index.html
// for typos like /api/foobar, masking real bugs.
// ============================================================
...
// Global error handler — catch anything that fell through (#15)
// Respects body-parser/multer errors (which carry their own statusCode) and
// only falls back to a generic 500 for unexpected exceptions.
// eslint-disable-next-line no-unused-vars
app.use((err, req, res, next) => {
  if (err && err.message && /CORS:|allowed/i.test(err.message)) {
    return res.status(400).json({ error: err.message });
  }
  if (err instanceof multer.MulterError) {
    return res.status(400).json({ error: err.message });
  }
  // body-parser errors carry { expose: true, statusCode: 400, type: 'entity.parse.failed' }
  if (err && err.expose && err.statusCode && err.statusCode < 500) {
    return res.status(err.statusCode).json({ error: "Bad request" });
  }
